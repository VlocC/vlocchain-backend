...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your vlocchain-backend account.<br>
        ...
        &mdash; vlocchain-backend Team
      `
      return sendMail({ toEmail: email, subject: 'vlocchain-backend - Password Reset', content })
    })
